...
int UsbDmxOut::open()
{
	...
	for (n = 0; n < MAXINTERFACES; n++)
	{
		...
		if (m_device[n] != -1)
		{
			txt.sprintf("Peperoni USB2DMX already open%d",
				    m_firstDeviceID+n);
	...
	{
		qDebug("Error: no valid USB2DMX devices could be opened! " \
		       "USB-DMX output is not available!");
	}
	else 
	{
		for (n = m_firstDeviceID; n <= m_lastDeviceID; n++)
		{
			txt.sprintf("USB2DMX Interface %d available for output",
				    n); 
			qDebug(txt);
		}
...
int UsbDmxOut::writeRange(t_channel address, t_value* values, t_channel num)
{
	...
	while (num > 0)
	{
		qDebug(txt.sprintf("writeRange:Universe #%d   "\
				   "FirstChannel #%d   Num #%d",
				   device, firstChannel, num));

		if (device > (m_lastDeviceID - m_firstDeviceID - 1))
		{
			qDebug("Error: UsbDmxOut::writeRange() tried to " \
			       "write to a non-existing device!");
			_mutex.unlock();
			return -1;
		}
		else
