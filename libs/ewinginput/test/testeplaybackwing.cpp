...
int main(int argc, char** argv)
{
	...
	QByteArray data(playbackData());

	QUdpSocket* socket = new QUdpSocket(&qapp);
	socket->writeDatagram(data.data(), data.size(),
				QHostAddress::Broadcast, 3330);
